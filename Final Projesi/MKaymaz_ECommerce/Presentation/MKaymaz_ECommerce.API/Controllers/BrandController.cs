...
namespace MKaymaz_ECommerce.API.Controllers
{
    ...
    public class BrandController : BaseApiController<BrandController>
    {
        ...
        public async Task<ActionResult<WebApiResponse<List<BrandResponseDto>>>> GetBrands()
        {
            ...
            //var brandResult = _mapper.Map<List<BrandResponseDto>>(await _brandRepository.GetByDefault(x => x.Id != System.Guid.Empty));
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult<WebApiResponse<BrandResponseDto>>> PutBrand(Guid id, BrandRequestDto request)
        {
            //UserResponseDto user = WorkContext.CurrentUser;
            if (id != request.Id)
                return BadRequest();
