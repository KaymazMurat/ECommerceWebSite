...
namespace MKaymaz_ECommerce.API.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<WebApiResponse<UserResponseDto>> Login([FromQuery] LoginRequestDto request)
        {
            ...
            return new WebApiResponse<UserResponseDto>(false, ModelState.Values.SelectMany(x => x.Errors).Select(x => x.ErrorMessage).ToList().ToString());
        }
        ...
        private GetAcceessTokenDto GetAcceessToken(UserResponseDto rm, string password)
        {
            ...
            {
                TokenType = "MKaymaz_ECommerceAccessToken",
                AccessToken = new JwtSecurityTokenHandler().WriteToken(jwtSecurityToken),
                Expires = ticks,
                RefreshToken = $"{rm.Email}_{password}_{ticks}".Encrypt() // Şifrele kısmı kaldırılabilir....
            };
        }
