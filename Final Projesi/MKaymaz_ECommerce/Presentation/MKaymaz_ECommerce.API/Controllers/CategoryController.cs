...
namespace MKaymaz_ECommerce.API.Controllers
{
    ...
    public class CategoryController : BaseApiController<CategoryController>
    {
        ...
        public async Task<ActionResult<WebApiResponse<List<CategoryResponseDto>>>> GetCategories()
        {
            ...
            //var categoryResult = _mapper.Map<List<CategoryResponseDto>>(await _categoryRepository.GetByDefault(x => x.Id != System.Guid.Empty));
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult<WebApiResponse<CategoryResponseDto>>> PutCategory(Guid id, CategoryRequestDto request)
        {
            //UserResponseDto user = WorkContext.CurrentUser;
            if (id != request.Id)
                return BadRequest();
